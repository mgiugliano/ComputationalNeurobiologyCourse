...
N  = Int64(T/Δt) # corresponding number of steps
t  = (1:N)*Δt    # time axis, ms

R  = zeros(N,1)  # allocated as an array filled of zeros
E  = zeros(N,1)  # allocated as an array filled of zeros
I  = zeros(N,1)  # allocated as an array filled of zeros
α  = zeros(N,1)  # allocated as an array filled of zeros

R[1] = 1.        # Initial condition: all resources are as "R"
E[1] = 0.        # Initial condition: all resources are as "R", none as "E"
I[1] = 0.        # Initial condition: all resources are as "R", none as "I"
...
RR  = zeros(N,1)  # allocated as an array filled of zeros
EE  = zeros(N,1)  # allocated as an array filled of zeros

RR[1] = 1.        # Initial condition: all resources are as "R"
...
delay  = 10 * τi2
events = delay:ISI:(M*ISI + delay) # by def, set of regular inter-spike intervals

u  = U     # Initialization..

out  = zeros(M,1)  # allocated as an array filled of zeros

out[1] = A * U;
events = sort(events);  # All events [s],  must be increasing in time..

# Actual numerical impelmentation of the iterative update rule --------------------------------------
for i=2:M                            # Loop across events, starting from the 2nd...
 Δ = events[i] - events[i-1]               # definition of inter-spike interval
 global r2 = (r2 * (1. - u) - 1.) * exp(-Δ/τD2) + 1. # STP (depression), iter expression..
...
AA    = [1., -exp(-Δt/τi2)]; # Refer to the documentation of "filt!()" - these are coefficients
