...
# ╔═╡ 24a16c06-6e70-4752-8f4d-8f008712e222
begin
	using Plots, PlutoUI
		plotly();
end

# ╔═╡ ca3e6810-3b28-11ec-12f2-5ff087a58119
md"""
# Example of a First-order O.D.E.
...
We solve it by 1) first discretizing the independent variable $x$ (i.e. into discrete points, uniformly spaced of $\Delta x$) and then 2) approximating the first derivative, as we learned in class:
...
# ╔═╡ 7e6943b1-fbbe-4a77-8637-5ac905ad7dff
md""" ### Play with the choice of the discretization step size (Δx)

 Δx = $(@bind Δx Slider(0.0007:0.0001:0.03, default=0.01, show_value=true))"""

# ╔═╡ d32ff7ba-4c1d-4a90-bfc1-dd4b239b8de5
md"""
The smaller the Δx, the higher the accuracy of the numerical solution (below). 
"""

# ╔═╡ 3efd5442-603e-4988-8dce-5d93988c68ab
begin
	
 T = 0.5 					# Maximal value of x, to calculate the solution  
 N   = Int64(ceil(T / Δx))  # Number of integration steps, given the choice of Δx 

 f = zeros(N,1); 			# Initialization of an array of N elements with 0s
	
	
 # Euler method ---------------------------------------------------------------
 x   = 0:Δx:(N-1)*Δx;    # This is the 'x-axis' for the numerical solution
