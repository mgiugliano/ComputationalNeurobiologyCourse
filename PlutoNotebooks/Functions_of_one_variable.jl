...
# ╔═╡ 4c55871c-e378-4da3-b7a6-698ff704695b
begin
	using Plots, PlutoUI
	plotly();
end

# ╔═╡ 30532876-35a1-11ec-02e0-d34da86493a7
md"""# *Computational Neurobiology*
## Math Refresher - Functions of one variable

A (mathematical) **function** can be imagined as the physical box depicted in the sketch below. When an element is offered as its input, an element is then istantaneously produced in output. Importantly, for a certain input element $x$ only one specific output element $y$ is returned. 

![Image of a function](https://upload.wikimedia.org/wikipedia/commons/3/3b/Function_machine2.svg)

Emphasising such a precise correspondence between input and output, the output element is sometimes also written as $f(x)$ and it reads as "*function of $x$*".


### Independent and dependent variables

Instead of specifying one by one the inputs $x$ and output $y$ (or $f(x)$) elements, we have used here generic symbols. These denote [variables](https://en.wikipedia.org/wiki/Variable_(mathematics)), which are identifiers (or *containers*) that can assume distinct values. The input variable $x$ is called *independent*, to distinguish it from the output $f(x)$ that is, by definition, dependent on the choice of the specific element of $x$ to which it is associated. We note that the use of letters $x$, $y$, and $f$ is arbitrary: those or other letters can (and will) be used interchangably, provided that references and consistency are maintained. 
...
As this is not a math course, our goal is turning such elementary concepts into more intuitive concepts. An efforts is requested to link mathematical formulations to graphs, so that formulae will perhaps no longer  appear "dry". Few notable operation on functions should be appreciated and mastered visually and the following subsections aims precisely at that.


"""

# ╔═╡ 041283e5-185b-4650-99cd-6070d0486b24
md"""
...
# ╔═╡ e4e7a1be-6497-46f1-8e2e-664cc9bca1f3
begin
	md""" a = $(@bind a Slider(-1.5:0.1:1.5, default=0, show_value=true))"""
end

# ╔═╡ bf2e97d0-649d-4320-91ba-659e44324bc4
begin
 x = -20:0.1:20	
 y = sin.(x)./x .+ a;
	
 mystr = (a>=0) ? "f(x) + $(a)" : "f(x) - $(-a)"
...
# ╔═╡ de3f39d7-3d09-4114-97f8-a0349b0ade14
md"""### How adding or subtracting a number to the independent variable changes the graph of a function
...
# ╔═╡ c89e0a72-cfc8-484b-96c0-1e8d1bfdbb23
begin
	md""" a = $(@bind b Slider(-10:1:10, default=0, show_value=true))"""
end

# ╔═╡ 3074a53b-7d47-44d1-9c03-7fc15c8f8aa3
begin
 x2 = -20:0.1:20	
 y2 = sin.(x2 .+ b)./(x2 .+ b);
	
 mystr2 = (b>=0) ? "f(x + $(b))" : "f(x - $(-b))"
...
# ╔═╡ adc8b28b-edc5-4bbf-9d57-d4f62d9ed379
md"""
...
# ╔═╡ 7b980a04-bfc9-4d2b-9915-a6569ed9b781
begin
	md""" a = $(@bind c Slider(-2:0.1:4, default=1, show_value=true))"""
end

# ╔═╡ 4a83fc18-6e17-404b-8c57-668df33ab699
begin
 x3 = -20:0.1:20	
 y3 = c .* sin.(x3)./x3;
	
mystr3 = "$(c) f(x)";  # This is a string for the graph legend
...
# ╔═╡ f4b842b3-a64e-45fa-a788-46f30a13fc2a
md"""### How multiplying the argument for a number changes the graph of a function
...
# ╔═╡ 17d368be-3d28-4051-8036-18901772073e
begin
	md""" a = $(@bind d Slider(-2:0.1:4, default=1, show_value=true))"""
end

# ╔═╡ daaaf5d3-dee6-46d6-8f2b-bbed2468e5c2
begin
 x4 = -20:0.1:20	
 y4 = sin.(d .* x4)./(d .* x4);
	
mystr4 = "f($(d) x)";  # This is a string for the graph legend
...
# ╔═╡ 2e565697-605b-460f-9469-5c565aec4e51
md"""
...
# ╔═╡ 162d0074-9005-42d2-8970-fa7d398bd224
begin
	md""" a = $(@bind e Slider(-1.5:0.1:1.5, default=1, show_value=true))"""
end

# ╔═╡ 3328fd1e-996a-4bbd-9ed1-5c601806cced
begin
 x5 = -20:1:20	
 y5 = e .* ones(41,1);
	
mystr5 = "$(e)";  # This is a string for the graph legend
...
# ╔═╡ 14af26e2-71f0-4e6d-bfd5-76ae5f8936b5
md"""
### The function *straight line*: $f(x) = m x + a$
"""

# ╔═╡ fbd64372-961b-493b-977e-28c3aa830235
begin
	md""" m = $(@bind m Slider(-1:0.05:1, default=0.3, show_value=true))"""
end

# ╔═╡ 03dc1575-6f2c-42c5-bf16-2d11c6a4e256
begin
	md""" p = $(@bind p Slider(-2:0.2:4, default=0, show_value=true))"""
end

# ╔═╡ 40395174-c396-48ed-b7e3-9075fc3994ef
begin
 x6 = -40:1:40	
 y6 = m .* x6 .+ p;
	
mystr6 = "$(m) x + $(p)";  # This is a string for the graph legend
...
# ╔═╡ 029b06e0-bedc-4cde-8651-2bb6f7bb30a4
md"""
### The function *exponential*: $f(x) = e^{x/c} + a$
"""

# ╔═╡ 19a493c6-434e-492a-8ad5-4a737325aa65
begin
	md""" c = $(@bind cc Slider(-2:0.2:4, default=0.2, show_value=true))"""
end

# ╔═╡ 45b38c12-a2e9-486b-bb77-07063b1b2c3f
begin
	md""" a = $(@bind aa Slider(-2:0.1:2, default=-1., show_value=true))"""
end

# ╔═╡ abd296df-3f9b-441d-b7bf-b852c1623c39
begin
 x7 = -4:.05:4	
 y7 = exp.(x7 ./ cc) .+ aa;
	
mystr7 = "exp(x/$(cc)) + $(aa)";  # This is a string for the graph legend
...
# ╔═╡ 9e4f8a4f-da6b-4425-ab53-1591a3406b41
md"""
### The function *exponential* (again): $f(x) = - e^{-x/c} + a$
"""

# ╔═╡ 2d365af0-1d5c-438c-8488-5d3457f1381c
begin
	md""" c = $(@bind ccc Slider(0.1:0.2:3.9, default=1, show_value=true))"""
end

# ╔═╡ b3658d1b-72af-43d9-8ba5-14cb02d64f27
begin
	md""" a = $(@bind aaa Slider(-1:0.1:4, default=1., show_value=true))"""
end

# ╔═╡ c525a53f-2016-4b02-9087-a0f6f2b8530b
begin
 x8 = -4:.05:4	
 y8 = -exp.(-x8 ./ ccc) .+ aaa;
	
mystr8 = "-exp(x/$(ccc)) + $(aaa)";  # This is a string for the graph legend
...
# ╔═╡ dba2837a-3443-4f53-b1c4-d085c7a4faff
md"""
### The function *logarithm*: $f(x) = log(a x)$
"""

# ╔═╡ 1bc71221-1b6f-437d-8dd2-32c3b1979b26
begin
	md""" a = $(@bind aaaa Slider(0.05:0.2:2.2, default=1., show_value=true))"""
end

# ╔═╡ 213edf9d-5400-462b-8dd3-86036a9f94d3
begin
 x9 = 0.001:.05:4	
 y9 = log.(aaaa.*x9);

 mystr9 = "log( $(aaa) x)";  # This is a string for the graph legend
